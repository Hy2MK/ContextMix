...
parser = argparse.ArgumentParser(description='ContextMix PyTorch CIFAR-10, CIFAR-100 and ImageNet-1k Training')
parser.add_argument('--net_type', default='pyramidnet', type=str, help='networktype: resnet, and pyamidnet')
...
parser.add_argument('--dataset', dest='dataset', default='imagenet', type=str, help='dataset (options: cifar10, cifar100, and imagenet)')
parser.add_argument('--no-verbose', dest='verbose', action='store_false', help='to print the status at every iteration')
parser.add_argument('--alpha', default=300, type=float, help='number of new channel increases per depth (default: 300)')
...
parser.add_argument('--mix_prob', default=0, type=float, help='cutmix probability')
...
def main():
    ...
    if args.net_type == 'resnet':
        model = ResNet(args.dataset, args.depth, numberofclass, args.bottleneck)
    elif args.net_type == 'pyramidnet':
        model = PyramidNet(args.dataset, args.depth, args.alpha, numberofclass, args.bottleneck)
    elif args.net_type == 'pyramidnet_shakedrop':
        model = ShakePyramidNet(depth=args.depth, alpha=args.alpha, label=numberofclass)
    else:
    ...
    with open("./output_rz_cs.txt", "a") as f1:
        print('Best accuracy (top-1 and 5 error):', best_err1, best_err5, file=f1)
    ...
    for i, (input, target) in enumerate(tqdm(train_loader)):
        ...
        if args.beta > 0 and r < args.mix_prob:
            ...
            if bbox_w > 2 and bbox_h > 2:
                ...
                input_rz = nn.functional.interpolate(input, size=(bbox_w, bbox_h), mode='nearest')
                input[:, :, bbx1:bbx2, bby1:bby2] = input_rz[rand_index]
                output = model(input)

                # adjust lambda to exactly match pixel ratio
                lam = 1 - (bbox_w * bbox_h / input_size)

                Lamda.update(lam.item(), input.size(0))
...
def save_checkpoint(state, is_best, filename='checkpoint_rz_cs.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, 'runs/%s/' % (args.expname) + 'model_best_rz_cs.pth.tar')
